-- 006_seed_demo_vehicles_rpc.sql
-- Purpose: Enable one-click demo vehicle seeding without admin login via a security-definer RPC.

create or replace function public.seed_demo_vehicles()
returns jsonb
language plpgsql
security definer
...
as $$
declare
  vehicle_count integer := 8;
  image_count integer := 24;
begin
...
revoke all on function public.seed_demo_vehicles() from public;
grant execute on function public.seed_demo_vehicles() to anon, authenticated, service_role;

-- Optional immediate seed when migration runs.
select public.seed_demo_vehicles();
