  ...
  const handleConfirm = async () => {
    ...
    try {
      const result = await createBooking({
        ...
        pickup_location: pickupLocation, notes, coupon_code: couponCode || null, discount_percent: discountPercent, discount_amount: quote?.discountAmount || 0,
          ...
          <section className="booking-form-card space-y-4 rounded-3xl border border-[#d5e0db] bg-[linear-gradient(165deg,#ffffff,#f9f4eb)] p-4 shadow-[0_14px_30px_rgba(10,31,34,0.08)] sm:p-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="grid gap-1">
                ...
                <input type="text" value={pickupLocation} onChange={(e) => setPickupLocation(e.target.value)} required placeholder="Enter pickup location" className={inputCls} />
              </label>
              <label className="grid gap-1">
                <span className="text-[12px] font-semibold text-[#315154]">Dropoff Location <span className="text-rose-600">*</span></span>
                <input type="text" value={dropoffLocation} onChange={(e) => setDropoffLocation(e.target.value)} required placeholder="Enter dropoff location" className={inputCls} />
              </label>
            </div>
