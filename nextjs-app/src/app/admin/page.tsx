...
export default function Overview() {
  ...
  const [revenueByDay, setRevenueByDay] = useState<{ labels: string[]; data: number[] }>({ labels: [], data: [] });
  const [fleetMix, setFleetMix] = useState<{ labels: string[]; data: number[] }>({ labels: [], data: [] });
  const [utilizationData, setUtilizationData] = useState<{ labels: string[]; booked: number[]; available: number[] }>({ labels: [], booked: [], available: [] });
    ...
    async function load() {
      try {
        ...
        const revArr = await Promise.all(days.map(async (day) => {
          ...
          return (dayBookings || []).reduce((s: number, b: any) => s + Number(b.paid_amount || 0), 0);
        }));
        setRevenueByDay({ labels: dayLabels, data: revArr });

        const { data: allVehicles } = await supabase.from('vehicles').select('category');
        const catMap: Record<string, number> = {};
        (allVehicles || []).forEach((v: any) => { const c = v.category || 'Other'; catMap[c] = (catMap[c] || 0) + 1; });
        ...
        (bookedVehicles || []).forEach((b: any) => { const c = b.vehicles?.category || 'Other'; bookedMap[c] = (bookedMap[c] || 0) + 1; });
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
        ...
        <section className={`${panel} xl:col-span-6 p-4 sm:p-5`}>
          <div className="mb-3 flex items-center justify-between">
            ...
            <button onClick={() => router.push('/admin/bookings')} className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-slate-100 dark:border-white/10 dark:text-slate-200 dark:hover:bg-white/10">View log</button>
          </div>
...
function WorkshopMini({ icon, label, count, color, onClick }: { icon: string, label: string, count: number, color: 'amber' | 'blue' | 'rose', onClick: () => void }) {
  const bg: Record<string, string> = { amber: 'border-amber-200 bg-amber-50 dark:border-amber-500/30 dark:bg-amber-500/10', blue: 'border-blue-200 bg-blue-50 dark:border-blue-500/30 dark:bg-blue-500/10', rose: 'border-rose-200 bg-rose-50 dark:border-rose-500/30 dark:bg-rose-500/10' };
  const txt: Record<string, string> = { amber: 'text-amber-700 dark:text-amber-300', blue: 'text-blue-700 dark:text-blue-300', rose: 'text-rose-700 dark:text-rose-300' };
  return (
