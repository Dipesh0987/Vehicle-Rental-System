...
export default function AdminReports() {
  ...
  useEffect(() => {
    (async () => {
      const [{ data: p }, { data: b }, { data: v }] = await Promise.all([
        ...
        supabase.from('bookings').select('id, vehicle_id, total_amount, status, created_at, vehicles(id, name, brand, model, category)'),
      ...
      (p || []).forEach((payment: any) => {
        const booking = (b || []).find((bk: any) => bk.id === payment.booking_id);
  ...
  const totalRevenue = useMemo(() => completed.reduce((s: number, p: any) => s + Number(p.amount || 0), 0), [completed]);
  ...
  const revenueChart = useMemo(() => {
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      ...
      const dayTotal = completed.filter((p: any) => (p.created_at || '').slice(0, 10) === iso).reduce((s: number, p: any) => s + Number(p.amount || 0), 0);
      data.push(dayTotal);
    }
    return { labels, data };
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)}
            className="rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-sm font-semibold dark:border-white/10 dark:bg-white/5 dark:text-slate-200" />
          <button 
            onClick={() => exportToCSV(vehicleRevenue.map((v: any) => ({
            ...
            <section className={`${panel} p-4 sm:p-5`}>
              <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button 
                    onClick={() => exportToCSV(vehicleRevenue.map((v: any, i: number) => ({
                      Rank: i + 1,
                      Vehicle: `${v.brand} ${v.model}`,
                      Category: v.category,
                      'Total Revenue': v.total_revenue,
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm">
                  ...
                  <tbody>
                    {vehicleRevenue.map((v: any, i: number) => (
