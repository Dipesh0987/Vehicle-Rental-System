            ...
            <tr data-contact-row-id="${m.id}" class="cursor-pointer transition hover:bg-slate-50/60 dark:hover:bg-white/5 ${m.status === 'unread' ? 'bg-amber-50/40 dark:bg-amber-500/5' : ''} ${selectedIds.has(m.id) ? 'bg-[#e8f5f3] dark:bg-[#1a3a38]' : ''}">
              ...
              <td class="px-4 py-3 text-right" data-actions-cell>
                <div class="inline-flex gap-1">
                  ...
                  ${m.status !== 'archived' ? `<button data-contact-action="archived" data-contact-id="${m.id}" class="rounded-lg p-1.5 text-slate-500 transition hover:bg-slate-200 hover:text-slate-700 dark:hover:bg-white/10 dark:hover:text-slate-300" title="Archive"><span class="material-symbols-outlined text-[18px]">archive</span></button>` : `<button data-contact-action="read" data-contact-id="${m.id}" class="rounded-lg p-1.5 text-slate-500 transition hover:bg-amber-100 hover:text-amber-700 dark:hover:bg-amber-500/20 dark:hover:text-amber-400" title="Unarchive"><span class="material-symbols-outlined text-[18px]">unarchive</span></button>`}
