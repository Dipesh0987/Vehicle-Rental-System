...
import { KeyRound, Edit2, Download, X, Plus, Trash2 } from 'lucide-react';
import { QRCodeSVG } from 'qrcode.react';

const COMPANY = {
  name: 'ASSELF DRIVE',
  address: 'Banasthali, Kathmandu, Nepal',
  website: 'www.asselfdrive.com',
  email: 'info@asselfdrive.com',
  phone: '+977 970-452-0781',
};

// Edit mode is shared via context so EditableField can live at module scope.
// (Defining EditableField inside the component caused inputs to remount on every
//  keystroke and lose focus after a single character.)
const EditModeContext = createContext(false);

const EditableField = ({ value, onChange, type = 'text', className = '', placeholder = '' }: { value: any, onChange: (v: any) => void, type?: string, className?: string, placeholder?: string }) => {
...
const formatRs = (n: number) => `Rs. ${(n || 0).toLocaleString('en-IN')}`;
    ...
    <div className="min-h-screen bg-gray-100 p-4 font-sans">
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-8 mt-12 items-end">
          ...
          <div className="text-center pb-2">
            <div className="border-b border-black w-48 mx-auto mb-2"></div>
            <p className="text-sm font-medium">Authorized by ASSelf Drive</p>
          </div>
        </div>
      </div>
...
export function InvoiceApp({ booking }: InvoiceProps) {
