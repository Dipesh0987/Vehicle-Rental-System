...
import { validateNepalPhone, validateEmail, validateName } from '@/lib/validations';
...
export default function VendorEnquiry() {
  ...
  const validateForm = () => {
    ...
    // Phone validation - Nepal format (97 or 98, 10 digits)
    const phoneValidation = validateNepalPhone(form.phone);
  ...
  const focusFirstError = (errs: Record<string, string>) => {
    const order = ['full_name', 'business_name', 'email', 'phone', 'city', 'fleet_count', 'service_type', 'price_min', 'price_max', 'images', 'terms_accepted'];
    const firstKey = order.find((k) => errs[k]);
    if (firstKey && typeof document !== 'undefined') {
      const el = document.getElementById(`venfield-${firstKey}`) as HTMLElement | null;
      if (el) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitted(true);
      toast.success('Your vendor enquiry has been submitted!');
    } catch (err: any) {
      console.error('Submission error:', err);
      const friendly = 'Something went wrong while submitting your enquiry. Please check your connection and try again.';
      setErrors({ ...errors, submit: friendly });
                  ...
                  <form onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                      <div>
                        ...
                        <input id="venfield-full_name" type="text" value={form.full_name} onChange={(e) => {
                      ...
                      <div>
                        ...
                        <input id="venfield-business_name" type="text" value={form.business_name} onChange={update('business_name')} placeholder="Your company/business name" className={inputCls} />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                      <div>
                        ...
                        <input id="venfield-email" type="email" value={form.email} onChange={update('email')} placeholder="your@email.com" className={inputCls} />
                        {errors.email && <p className="text-rose-500 text-xs mt-1">{errors.email}</p>}
                        <p className="text-[10px] text-muted mt-1">e.g., name@gmail.com, name@yahoo.com</p>
                      </div>
                      <div>
                        <label className="block font-poppins text-[13px] font-semibold text-ink dark:text-[#cbd5e1] mb-1.5">
                          Phone Number <span className="text-accent">*</span>
                        </label>
                        <input id="venfield-phone" type="tel" value={form.phone} onChange={(e) => {
                          const val = e.target.value.replace(/[^\d]/g, '');
                          if (val.length <= 10) {
                            setForm({ ...form, phone: val });
                            if (errors.phone) setErrors({ ...errors, phone: '' });
                          }
                        }} placeholder="98XXXXXXXX" maxLength={10} className={inputCls} />
                        {errors.phone && <p className="text-rose-500 text-xs mt-1">{errors.phone}</p>}
                        <p className="text-[10px] text-muted mt-1">Nepal: starts with 97 or 98, 10 digits</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                      <div>
                        ...
                        <input id="venfield-city" type="text" value={form.city} onChange={update('city')} placeholder="e.g., Kathmandu" className={inputCls} />
                      ...
                      <div>
                        ...
                        <input id="venfield-fleet_count" type="number" min="1" value={form.fleet_count} onChange={update('fleet_count')} placeholder="e.g., 5" className={inputCls} />
                    ...
                    <div className="mb-5">
                      ...
                      <select id="venfield-service_type" value={form.service_type} onChange={update('service_type')} className={inputCls}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                      <div>
                        ...
                        <input id="venfield-price_min" type="number" min="0" value={form.price_min} onChange={update('price_min')} placeholder="e.g., 3000" className={inputCls} />
                      ...
                      <div>
                        ...
                        <input id="venfield-price_max" type="number" min="0" value={form.price_max} onChange={update('price_max')} placeholder="e.g., 8000" className={inputCls} />
                    ...
                    <div className="mb-6 p-4 rounded-xl bg-[#F8F9F7] dark:bg-[#1c2a2e] border border-[#e8ebe6] dark:border-white/[0.08]">
                      <label className="flex items-start gap-3 cursor-pointer">
                        <input id="venfield-terms_accepted" type="checkbox" checked={form.terms_accepted} onChange={update('terms_accepted')} className="mt-1 w-5 h-5 rounded border-[#dfe3dc] text-[#145f59] focus:ring-[#145f59] cursor-pointer" />
