...
export default function AdminRoles() {
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    if (createForm.role === 'super_admin' && admins.some((a: any) => a.role === 'super_admin')) {
      toast.warning('Only one Super Admin is allowed! Please select Admin or Staff role.');
      return;
    }
      ...
      if (authError) {
        if (authError.message?.includes('already registered')) {
          ...
          if (existingProfiles && existingProfiles.length > 0) {
            ...
            toast.success('Existing user updated successfully!');
          } else {
            toast.error('User already exists in auth but could not be found in profiles. Please contact support.');
            return;
          }
        } else {
          throw authError;
