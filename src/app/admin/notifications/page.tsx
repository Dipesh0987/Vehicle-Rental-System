...
export default function AdminNotifications() {
  ...
  const iconFor = (type: string) => {
    const m: Record<string, string> = { contact: 'mail', booking: 'event_note', payment: 'credit_card', vehicle: 'directions_car', maintenance: 'build', system: 'info', verification: 'verified', verification_queue: 'pending', verification_submission: 'person_search' };
    return m[type] || 'notifications';
  };

  const iconColor = (type: string) => {
    const m: Record<string, string> = { contact: 'text-violet-500', booking: 'text-blue-500', payment: 'text-emerald-500', vehicle: 'text-orange-500', maintenance: 'text-amber-500', system: 'text-slate-500', verification: 'text-teal-500', verification_queue: 'text-amber-600', verification_submission: 'text-teal-500' };
      ...
      {kycQueue && (
        <section className={`${panel} border-amber-300/70 bg-[linear-gradient(130deg,rgba(255,247,214,0.95),rgba(255,238,191,0.9))] p-4 sm:p-5 dark:border-amber-400/30 dark:bg-amber-500/10`}>
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <button onClick={() => router.push('/admin/customers')} className="rounded-full border border-amber-400 bg-white/90 px-3 py-1 text-xs font-bold uppercase tracking-[0.12em] text-amber-800 transition hover:bg-amber-50 dark:border-amber-300/40 dark:bg-amber-500/10 dark:text-amber-100">Review Now</button>
          </div>
        </section>
      )}
      ...
      <section className={`${panel} p-4 sm:p-5`}>
        <div className="space-y-2">
          {loading ? <p className="py-6 text-center text-sm text-slate-400 dark:text-slate-500">Loading…</p> : filtered.length === 0 ? (
            ...
            return (
              <div key={n.id}
                onClick={() => isClickable && router.push(target!)}
