...
export default function DamageClaims() {
  ...
  const fetchClaims = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('damage_claims')
        .select(`*, vehicles(id, name, vehicle_number, primary_image_url), bookings(id, booking_code, start_date, end_date)`)
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!form.vehicle_id || !form.damage_description || !form.total_damage_cost) {
      toast.warning('Please fill in vehicle, description, and damage cost');
      return;
    }
    setSaving(true);
    try {
      ...
      toast.error('Error creating claim: ' + err.message);
    } finally {
      setSaving(false);
    }
  };
...
ASSelf Car Rental`;
  ...
  const createMaintenanceFromClaim = async (claim: any) => {
    try {
      ...
      toast.success('Maintenance record created! You can track the repair in the Maintenance page.');
      router.push('/admin/maintenance');
    } catch (err: any) {
      toast.error('Error creating maintenance: ' + err.message);
    }
  };
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                ...
                <label className="block">
                  ...
                  <input type="date" value={form.damage_date} max={new Date().toISOString().slice(0, 10)} onChange={u('damage_date')} className={`mt-1 ${inp}`} />
                  <span className="text-[10px] text-slate-400">Cannot be a future date</span>
                </label>
              </div>
            </div>
          </section>
