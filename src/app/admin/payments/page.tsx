...
export default function AdminPayments() {
  ...
  const openDetail = async (p: any) => {
    setDetail(p);
    if (p.booking_id) {
      const { data } = await supabase.from('bookings').select('*, vehicles(name)').eq('id', p.booking_id).single();
      setBooking(data);
    } else { setBooking(null); }
  };

  const [bookingsForStats, setBookingsForStats] = useState<any[]>([]);
  ...
  const statusColor = (s: string) => {
    const c: Record<string, string> = { completed: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300', pending: 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300', initiated: 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300', failed: 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-300', expired: 'bg-slate-100 text-slate-600 dark:bg-slate-500/20 dark:text-slate-300', cancelled: 'bg-slate-100 text-slate-600 dark:bg-slate-500/20 dark:text-slate-300', refunded: 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300' };
        ...
        <div className={`${panel} p-4 sm:p-6`}>
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-3">
            <article className="rounded-2xl border border-slate-200 bg-white/80 p-4 dark:border-white/10 dark:bg-white/5 xl:col-span-2">
              ...
              {(booking?.payment_receipt_url || p.receipt_url) && (
                <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-500/30 dark:bg-emerald-500/10">
                  ...
                  <a href={p.receipt_url || booking?.payment_receipt_url} target="_blank" rel="noopener noreferrer" className="block max-w-sm">
                    <img 
                      src={p.receipt_url || booking?.payment_receipt_url} 
                      alt="Payment receipt" 
                      className="w-full max-h-[400px] rounded-lg border border-emerald-300 object-contain bg-white" 
                      onError={(e: any) => {
                        e.target.style.display = 'none';
                        e.target.parentElement.innerHTML = '<span class="text-xs text-emerald-600 p-2 block">Image failed to load. <a href="' + (p.receipt_url || booking?.payment_receipt_url) + '" target="_blank" class="underline">Try direct link</a></span>';
                      }}
                    />
                  </a>
                  <a href={p.receipt_url || booking?.payment_receipt_url} target="_blank" rel="noopener noreferrer" className="mt-1 inline-block text-xs font-semibold text-emerald-700 underline">View full size</a>
                </div>
              )}
              ...
              {p.status === 'pending' && (
                <div className="mt-4 flex flex-wrap gap-2">
                  <button onClick={async () => {
                    ...
                    if (p.booking_id) {
                      await supabase.from('bookings').update({ status: 'confirmed', payment_status: 'completed', is_paid: true, paid_amount: p.amount, remaining_amount: 0 }).eq('id', p.booking_id);
                    }
                    await fetch_();
                  ...
                  <button onClick={async () => {
                    ...
                    if (p.booking_id) {
                      await supabase.from('bookings').update({ status: 'cancelled', payment_status: 'rejected', is_paid: false }).eq('id', p.booking_id);
                    }
                    await fetch_();
        ...
        {loading ? <div className="p-8 text-center text-sm text-slate-400">Loading…</div> : paged.length === 0 ? <div className="p-8 text-center text-sm text-slate-400">No payments found.</div> : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {paged.map((p) => (
                  <tr key={p.id} className="border-b border-slate-100 transition hover:bg-slate-50 dark:border-white/5 dark:hover:bg-white/5">
                    <td className="py-3 pr-3 pl-4"><button onClick={() => openDetail(p)} className="font-mono text-xs font-bold text-[#1f7668] underline decoration-[#1f7668]/30 underline-offset-2 hover:text-[#185f54] dark:text-emerald-300">{p.transaction_code || p.id?.slice(0, 8)}</button></td>
                    ...
                    <td className="py-3 pr-3 text-xs font-semibold uppercase text-slate-600 dark:text-slate-300">{p.payment_method || p.method || 'cash'}</td>
                    <td className="py-3 pr-3">
                      <select 
                        value={p.status} 
                        onChange={async (e) => {
                          ...
                          if (p.booking_id) {
                            ...
                            await supabase.from('bookings').update({
                              ...
                              is_paid: isPaid,
                              paid_amount: isPaid ? p.total_booking_amount : p.amount,
                              remaining_amount: isPaid ? 0 : ((p.total_booking_amount || p.amount) - p.amount)
                            }).eq('id', p.booking_id);
                          }
                        ...
                        className={`w-[100px] rounded-lg border px-2 py-1 text-xs font-semibold outline-none cursor-pointer ${statusColor(p.status)}`}
                      >
                    ...
                    <td className="py-3 pr-3">
                      <div className="flex gap-1">
                        <button onClick={() => openDetail(p)} className="rounded-lg bg-slate-100 px-2 py-1 text-xs font-semibold hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600">View</button>
                      </div>
                    </td>
                  </tr>
                ))}
