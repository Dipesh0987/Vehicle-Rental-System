...
const shortId = (id: string) => { const s = String(id || ''); return s.length <= 12 ? s : `${s.slice(0, 8)}...${s.slice(-4)}`; };
const initials = (name: string) => String(name || '').trim().split(/\s+/).slice(0, 2).map((w) => w.charAt(0).toUpperCase()).join('') || 'CU';
const fmtDt = (v: string) => v ? new Date(v).toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' }) : '';
const verifyMeta = (s: string, isGuest = false) => {
...
const kycProgress = (key: string) => ({ approved: 100, pending: 60, rejected: 25 }[key] || 10);
const locText = (c: any) => [c.city, c.country].filter(Boolean).join(', ') || 'Location not provided';
...
export default function AdminCustomers() {
  ...
  const fetch_ = async () => {
    ...
    const guestCustomers = (guestBookings || []).map((booking: any, index: number) => ({
      id: `guest-${booking.id}`,
    ...
    const allCustomers = [
      ...(profiles || []).map((p: any) => ({ ...p, is_guest: false })),
      ...guestCustomers
  ...
  const handleVerification = async (userId: string, status: string) => {
    ...
    await fetch_();
    if (detail?.id === userId) setDetail((prev: any) => prev ? { ...prev, verification_status: status } : null);
    setUpdating(false);
  };
  ...
  const renderDetail = (row: any) => {
    ...
    const ActionBtn = ({ action, label, cls }: { action: string, label: string, cls: string }) => (
  ...
  const FocusCard = ({ c, i }: { c: any, i: number }) => {
