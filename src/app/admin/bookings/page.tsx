...
export default function AdminBookings() {
  ...
  const fetchVehicles = async () => {
    ...
    try {
      ...
      toast.error('Failed to load vehicles: ' + (err.message || 'Unknown error'));
    } finally {
  ...
  const handleCreateBooking = async (e: React.FormEvent) => {
    ...
    // Validate fields with friendly messages + scroll-to-error
    const errs: Record<string, string> = {};
    if (!createForm.vehicle_id) errs.vehicle_id = 'Please select a vehicle.';
    const nameV = validateName(createForm.customer_name);
    if (!nameV.valid) errs.customer_name = nameV.message;
    if (!createForm.customer_phone) errs.customer_phone = 'Please enter the customer phone number.';
    else { const pv = validateNepalPhone(createForm.customer_phone); if (!pv.valid) errs.customer_phone = pv.message; }
    if (createForm.customer_email) { const ev = validateEmail(createForm.customer_email); if (!ev.valid) errs.customer_email = ev.message; }
    if (!createForm.start_date) errs.start_date = 'Please choose a start date.';
    else { const sv = validateNotPastDate(createForm.start_date); if (!sv.valid) errs.start_date = sv.message; }
    if (!createForm.end_date) errs.end_date = 'Please choose an end date.';
    else if (createForm.start_date) { const dv = validateDateRange(createForm.start_date, createForm.end_date); if (!dv.valid) errs.end_date = dv.message; }
    const pickV = validateLocation(createForm.pickup_location);
    if (!pickV.valid) errs.pickup_location = pickV.message;
    if (createForm.dropoff_location) { const dropV = validateLocation(createForm.dropoff_location); if (!dropV.valid) errs.dropoff_location = dropV.message; }

    setCreateErrors(errs);
    if (Object.keys(errs).length > 0) {
      const order = ['vehicle_id', 'customer_name', 'customer_phone', 'customer_email', 'start_date', 'end_date', 'pickup_location', 'dropoff_location'];
      const firstKey = order.find((k) => errs[k]);
      if (firstKey) {
        const el = document.getElementById(`createfield-${firstKey}`) as HTMLElement | null;
        if (el) { el.scrollIntoView({ behavior: 'smooth', block: 'center' }); setTimeout(() => { try { el.focus({ preventScroll: true }); } catch {} }, 250); }
            ...
            <form onSubmit={handleCreateBooking} className="space-y-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <label className="block space-y-1"><span className="text-xs font-semibold">Customer Name *</span>
                  <input id="createfield-customer_name" type="text" value={createForm.customer_name} onChange={(e) => {
                    const val = e.target.value;
                    if (/^[a-zA-Z\s'-]*$/.test(val) || val === '') { setCreateForm({ ...createForm, customer_name: val }); if (createErrors.customer_name) setCreateErrors({ ...createErrors, customer_name: '' }); }
                  }} required className={`${inp} ${createErrors.customer_name ? 'border-rose-400' : ''}`} />
                  {createErrors.customer_name && <span className="text-[11px] text-rose-600">{createErrors.customer_name}</span>}
                </label>
                <label className="block space-y-1"><span className="text-xs font-semibold">Customer Phone *</span>
                  <input id="createfield-customer_phone" type="tel" value={createForm.customer_phone} onChange={(e) => {
                    const val = e.target.value.replace(/[^\d]/g, '');
                    if (val.length <= 10) { setCreateForm({ ...createForm, customer_phone: val }); if (createErrors.customer_phone) setCreateErrors({ ...createErrors, customer_phone: '' }); }
                  }} required placeholder="98XXXXXXXX" maxLength={10} className={`${inp} ${createErrors.customer_phone ? 'border-rose-400' : ''}`} />
                  {createErrors.customer_phone && <span className="text-[11px] text-rose-600">{createErrors.customer_phone}</span>}
                </label>
                <label className="block space-y-1 sm:col-span-2"><span className="text-xs font-semibold">Customer Email</span>
                  <input id="createfield-customer_email" type="email" value={createForm.customer_email} onChange={(e) => { setCreateForm({ ...createForm, customer_email: e.target.value }); if (createErrors.customer_email) setCreateErrors({ ...createErrors, customer_email: '' }); }} className={`${inp} ${createErrors.customer_email ? 'border-rose-400' : ''}`} />
                  {createErrors.customer_email && <span className="text-[11px] text-rose-600">{createErrors.customer_email}</span>}
                </label>
                <label className="block space-y-1"><span className="text-xs font-semibold">Start Date *</span>
                  <input id="createfield-start_date" type="date" min={new Date().toISOString().split('T')[0]} value={createForm.start_date} onChange={(e) => { setCreateForm({ ...createForm, start_date: e.target.value }); if (createErrors.start_date) setCreateErrors({ ...createErrors, start_date: '' }); }} required className={`${inp} ${createErrors.start_date ? 'border-rose-400' : ''}`} />
                  {createErrors.start_date && <span className="text-[11px] text-rose-600">{createErrors.start_date}</span>}
                </label>
                <label className="block space-y-1"><span className="text-xs font-semibold">End Date *</span>
                  <input id="createfield-end_date" type="date" min={createForm.start_date || new Date().toISOString().split('T')[0]} value={createForm.end_date} onChange={(e) => { setCreateForm({ ...createForm, end_date: e.target.value }); if (createErrors.end_date) setCreateErrors({ ...createErrors, end_date: '' }); }} required className={`${inp} ${createErrors.end_date ? 'border-rose-400' : ''}`} />
                  {createErrors.end_date && <span className="text-[11px] text-rose-600">{createErrors.end_date}</span>}
                </label>
                ...
                <label className="block space-y-1"><span className="text-xs font-semibold">Pickup Location *</span>
                  <input id="createfield-pickup_location" type="text" value={createForm.pickup_location} onChange={(e) => { setCreateForm({ ...createForm, pickup_location: e.target.value }); if (createErrors.pickup_location) setCreateErrors({ ...createErrors, pickup_location: '' }); }} required placeholder="e.g. Banasthali, Kathmandu" className={`${inp} ${createErrors.pickup_location ? 'border-rose-400' : ''}`} />
                  {createErrors.pickup_location && <span className="text-[11px] text-rose-600">{createErrors.pickup_location}</span>}
                </label>
                <label className="block space-y-1"><span className="text-xs font-semibold">Dropoff Location</span>
                  <input id="createfield-dropoff_location" type="text" value={createForm.dropoff_location} onChange={(e) => { setCreateForm({ ...createForm, dropoff_location: e.target.value }); if (createErrors.dropoff_location) setCreateErrors({ ...createErrors, dropoff_location: '' }); }} placeholder="e.g. Airport, Kathmandu (optional)" className={`${inp} ${createErrors.dropoff_location ? 'border-rose-400' : ''}`} />
                  {createErrors.dropoff_location && <span className="text-[11px] text-rose-600">{createErrors.dropoff_location}</span>}
                </label>
