...
import { calculateEndDate, validateNotPastDate } from '@/lib/validations';
...
export default function VehicleDetails() {
  ...
  // Auto-calculate end date when pickup date or duration changes
  useEffect(() => {
    if (pickupDate && duration > 0) {
      const calculatedEnd = calculateEndDate(pickupDate, duration);
  ...
  const handlePickupDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validation = validateNotPastDate(newDate);
    if (!validation.valid) {
  ...
  const handleBook = () => {
    ...
    if (pickupDate) {
      const validation = validateNotPastDate(pickupDate);
      if (!validation.valid) {
        setDateError(validation.message);
        return;
      }
          ...
          <aside className="detail-card rounded-3xl border border-[#d5e0db] bg-white/95 p-5 shadow-[0_16px_30px_rgba(10,31,34,0.08)] xl:sticky xl:top-5">
            ...
            <div className="mt-4 grid gap-3">
              <label className="grid gap-1">
                ...
                <input type="date" value={pickupDate} onChange={handlePickupDateChange} min={today}
                  className="rounded-xl border border-[#d4dfda] bg-[#fbfdfc] px-3 py-2 text-[13px] text-[#1f4043] outline-none" />
                {dateError && <span className="text-[11px] text-rose-600">{dateError}</span>}
              </label>
              <label className="grid gap-1">
              ...
              {endDate && (
                <div className="rounded-xl border border-[#d6e2dd] bg-[#f0f7f4] px-3 py-2">
                  <span className="text-[11px] font-semibold text-[#476367]">Drop-off Date</span>
                  <p className="text-[13px] font-semibold text-[#1f4043]">{endDate}</p>
                </div>
              )}
