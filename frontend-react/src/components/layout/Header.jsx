...
import BookingsModal from '../ui/BookingsModal';
  ...
  return (
    <>
      <header className="vrs-theme-scope relative z-[90] mx-auto grid w-[95%] max-w-[1390px] grid-cols-[auto,1fr,auto] items-center gap-6 py-4">
        {/* Brand */}
        <Link to="/" className="group relative text-ink transition duration-200 hover:-translate-y-[1px] hover:text-[#164144]">
          <span className="vrs-brand-wordmark block bg-[linear-gradient(120deg,#0e2528,#123f43_42%,#1f6660)] bg-clip-text text-[24px] font-extrabold leading-none tracking-tight text-transparent lg:text-[34px]">
            RENT A VEHICLE
          </span>
          <span className="vrs-brand-tagline -mt-1 block text-[7px] font-semibold uppercase tracking-[0.16em] text-[#1B2E2F] dark:text-slate-300 lg:text-[11px]">Luxury Vehicle Service</span>
        ...
        <nav className="vrs-global-nav hidden items-center justify-start gap-10 pl-6 text-[16px] font-medium text-[#2F3336] lg:flex">
          <Link to="/" className={isActive('/') ? 'vrs-global-nav-link text-accent' : navLinkBase}>Home</Link>
          <Link to="/vehicles" className={isActive('/vehicles') || isActive('/booking') ? 'vrs-global-nav-link text-accent' : navLinkBase}>Vehicles</Link>
          <Link to="/contact" className={isActive('/contact') ? 'vrs-global-nav-link text-accent' : navLinkBase}>Contact</Link>
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          {!showAsLoggedIn ? (
            <div data-auth-guest className="hidden items-center gap-8 pr-8 text-[16px] text-[#204144] lg:flex">
              <Link to="/login" className="bg-[linear-gradient(currentColor,currentColor)] bg-[length:0%_2px] bg-[position:0_100%] bg-no-repeat transition-[color,transform,background-size] duration-200 hover:-translate-y-[1px] hover:bg-[length:100%_2px] hover:text-panel">Sign In</Link>
              <Link to="/registration" className="rounded-full bg-accent px-8 py-3 text-[16px] font-medium leading-none text-white transition duration-200 hover:-translate-y-[1px] hover:brightness-105 hover:shadow-[0_10px_22px_rgba(229,140,78,0.32)]">
                Sign Up
              </Link>
            </div>
          ) : (
            <div data-auth-user className="hidden items-center gap-3 lg:flex">
              ...
              <div ref={profileRef} className="relative">
                ...
                {/* Profile dropdown */}
                <div className={`absolute right-0 top-[calc(100%+10px)] z-[130] w-[280px] origin-top-right rounded-[16px] border border-[rgba(23,57,60,0.12)] bg-white p-2 shadow-[0_20px_45px_rgba(2,14,16,0.15)] transition duration-200 dark:border-white/10 dark:bg-[#1a2e30] ${profileOpen ? 'opacity-100 translate-y-0 scale-100 pointer-events-auto' : 'opacity-0 -translate-y-2 scale-95 pointer-events-none'}`}>
                  {/* User info */}
                  ...
                  <Link to="/profile-verification" onClick={() => setProfileOpen(false)} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-medium text-[#264447] transition hover:bg-[#f3f8f6] dark:text-white/80 dark:hover:bg-white/5">
                  ...
                  <button type="button" onClick={async () => { await signOut(); setProfileOpen(false); navigate('/'); }}
      ...
      <div className="vrs-theme-scope relative z-20 mx-auto flex w-[95%] max-w-[1390px] flex-wrap gap-2 pb-2 lg:hidden">
        <Link to="/" className={`rounded-full border px-3 py-1.5 text-[12px] font-semibold ${isActive('/') ? 'border-[#f1c8aa] bg-[#fff4eb] text-[#9f5825]' : 'border-[#d4ded9] bg-white/95 text-[#264447]'}`}>Home</Link>
        <Link to="/vehicles" className={`rounded-full border px-3 py-1.5 text-[12px] font-semibold ${isActive('/vehicles') || isActive('/booking') ? 'border-[#f1c8aa] bg-[#fff4eb] text-[#9f5825]' : 'border-[#d4ded9] bg-white/95 text-[#264447]'}`}>Vehicles</Link>
        <Link to="/contact" className={`rounded-full border px-3 py-1.5 text-[12px] font-semibold ${isActive('/contact') ? 'border-[#f1c8aa] bg-[#fff4eb] text-[#9f5825]' : 'border-[#d4ded9] bg-white/95 text-[#264447]'}`}>Contact</Link>
        ...
        {showAsLoggedIn && (
          <Link to="/profile-verification" className={`rounded-full border px-3 py-1.5 text-[12px] font-semibold ${isActive('/profile-verification') ? 'border-[#f1c8aa] bg-[#fff4eb] text-[#9f5825]' : 'border-[#d4ded9] bg-white/95 text-[#264447]'}`}>Verify</Link>
        )}
      </div>
