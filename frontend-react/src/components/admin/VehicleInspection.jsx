...
// Default parts - used immediately while fetching from DB
const DEFAULT_PARTS = [
...
const getInitialConditions = (partsArray) => {
  const initial = {};
...
export default function VehicleInspection({ booking, inspectionType, onComplete, onCancel }) {
  ...
  const [partConditions, setPartConditions] = useState(() => getInitialConditions(DEFAULT_PARTS));
  const [beforeInspection, setBeforeInspection] = useState(null);
  ...
  const fetchParts = async () => {
    // Don't block - we already have default parts loaded
    try {
      ...
      if (!error && data && data.length > 0) {
        // Only update if we got parts from DB
        setParts(data);
        setPartConditions(getInitialConditions(data));
      }
      // Otherwise keep using DEFAULT_PARTS
    } catch (err) {
      console.error('Fetch parts error:', err);
      // Keep using DEFAULT_PARTS
    }
  };
  ...
  const fetchBeforeInspection = async () => {
    ...
    if (data) {
      ...
      // Pre-fill with before inspection conditions
      const conditions = {};
  ...
  const updatePartCondition = (partId, field, value) => {
  ...
  const setAllPartsCondition = (condition) => {
    const updated = {};
    parts.forEach(p => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If after_trip, check for damages and create claim if needed
  ...
  const checkAndCreateDamageClaim = async (afterInspectionId) => {
    // Compare before and after conditions
    const damages = [];
    
    parts.forEach(p => {
      const beforeCond = beforeInspection?.inspection_items?.find(i => i.part_id === p.id)?.condition || 'good';
      const afterCond = partConditions[p.id]?.condition || 'good';
      
      // Damage detected if condition worsened
  ...
  const totalDamageCost = useMemo(() => {
    return Object.entries(partConditions).reduce((sum, [partId, data]) => {
