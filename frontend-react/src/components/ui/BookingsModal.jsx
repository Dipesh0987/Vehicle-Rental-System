...
const fmt = (v) => `NPR ${Number(v || 0).toLocaleString()}`;
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (guestBookings.length > 0) {
        // Fetch full details for guest bookings
            ...
            {!active ? (
              ...
              <>
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
                  <button type="button" onClick={() => { onClose(); navigate('/my-bookings'); }}
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {active.status === 'pending' && (
                      <button type="button" onClick={() => { onClose(); navigate(`/payment?booking=${active.id}`); }}
                    ...
                    {active.status !== 'cancelled' && active.status !== 'completed' && (
                      <button type="button" onClick={() => { onClose(); navigate(`/modify-booking?id=${active.id}`); }}
