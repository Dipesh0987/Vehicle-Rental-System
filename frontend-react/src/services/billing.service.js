...
// INVOICE CRUD
// =========================================================
export async function getInvoices({ status, search, from, to, limit = 50, offset = 0 } = {}) {
...
export async function createInvoiceFromBooking(booking) {
  const rentalDays = booking.rental_duration || Math.ceil((new Date(booking.end_date) - new Date(booking.start_date)) / 86400000) || 1;
...
// BILLING PAYMENTS
// =========================================================
export async function getBillingPayments({ invoiceId, status, method, from, to, limit = 50, offset = 0 } = {}) {
  const result = await safeQuery(async () => {
...
export async function createBillingPayment(payload) {
  const { data, error } = await supabase.from('billing_payments').insert(payload).select().single();
  if (error) throw error;

  // Update invoice amount_paid and outstanding
...
export async function verifyPayment(paymentId, verified, rejectionReason) {
...
// EXPENSES
// =========================================================
export async function getExpenses({ category, vehicleId, search, from, to, limit = 50, offset = 0 } = {}) {
  const result = await safeQuery(async () => {
...
export async function getVehicleFinances() {
  // Return empty array since vehicle_finances table doesn't exist
  return [];
}

export async function recalcVehicleFinance(vehicleId) {
  // Disabled - vehicle_finances table doesn't exist
  return null;
}
...
export async function getFinancialDashboard() {
  ...
  const safe = async (query) => { try { return await query; } catch (_) { return { data: null, count: 0 }; } };
  ...
  const todayRevenue = (todayInvoices || []).reduce((s, i) => s + Number(i.amount_paid || 0), 0);
  const monthlyRevenue = (monthInvoices || []).reduce((s, i) => s + Number(i.amount_paid || 0), 0);
  const yearlyRevenue = (yearInvoices || []).reduce((s, i) => s + Number(i.amount_paid || 0), 0);
  const totalExpenseAmt = (allExpenses || []).reduce((s, e) => s + Number(e.amount || 0), 0);
  const netProfit = yearlyRevenue - totalExpenseAmt;
  const totalOutstanding = (outstandingInvs || []).reduce((s, i) => s + Number(i.outstanding_balance || 0), 0);
  const totalQR = (qrPayments || []).reduce((s, p) => s + Number(p.amount || 0), 0);
  const totalCash = (cashPayments || []).reduce((s, p) => s + Number(p.amount || 0), 0);
...
export async function getRevenueChartData(months = 12) {
  const data = [];
  const now = new Date();
  const safe = async (query) => { try { return await query; } catch (_) { return { data: null }; } };
  for (let i = months - 1; i >= 0; i--) {
    ...
    const income = (incomeData || []).reduce((s, i) => s + Number(i.amount_paid || 0), 0);
...
export async function getExpenseReport({ from, to, category }) {
...
export async function getCustomerBillingHistory(customerId) {
  let invoices = [], payments = [], bookings = [];
  try {
  ...
  // Fallback: also fetch from base 'payments' table if billing_payments is empty
  if (payments.length === 0) {
    try {
      const { data } = await supabase.from('payments').select('*').eq('user_id', customerId).order('created_at', { ascending: false });
      if (data?.length > 0) {
        payments = data.map(p => ({ id: p.id, payment_date: p.created_at, amount: p.amount, payment_method: p.method || 'online_qr', payment_type: 'booking', verification_status: p.status === 'completed' ? 'verified' : p.status, reference_number: p.transaction_id }));
      }
    } catch (_) {}
  }
  // vehicle_bookings uses user_id for the customer
  const { data: bkData } = await supabase.from('vehicle_bookings').select('id, booking_code, vehicle_id, start_date, end_date, pickup_time, driver_option, base_amount, service_fee, tax_amount, discount_amount, total_amount, paid_amount, remaining_amount, status, payment_status, is_paid, coupon_code, customer_name, customer_email, customer_phone, notes, created_at, vehicles:vehicle_id(name, brand, category, vehicle_number)').eq('user_id', customerId).order('created_at', { ascending: false });
  bookings = bkData || [];
  // If still no billing payments, synthesize from paid bookings
  if (payments.length === 0) {
    payments = bookings.filter(b => b.is_paid || b.payment_status === 'completed').map(b => ({ id: b.id, payment_date: b.created_at, amount: b.paid_amount || b.total_amount, payment_method: 'cash', payment_type: 'booking', verification_status: 'verified' }));
  }
  const totalSpent = payments.filter(p => p.verification_status === 'verified').reduce((s, p) => s + Number(p.amount || 0), 0) || bookings.filter(b => b.is_paid || b.status === 'confirmed' || b.status === 'completed').reduce((s, b) => s + Number(b.paid_amount || b.total_amount || 0), 0);
  const totalOutstanding = invoices.reduce((s, i) => s + Number(i.outstanding_balance || 0), 0);
...
export async function updateBillingSetting(key, value) {
...
// AUDIT LOG
// =========================================================
export async function logAudit(action, module, entityType, entityId, previousValue, newValue, description) {
  try {
    const { data: session } = await supabase.auth.getSession();
    const userId = session?.session?.user?.id;
...
export async function getAuditLogs({ module, action, from, to, limit = 100, offset = 0 } = {}) {
  const result = await safeQuery(async () => {
