...
export function calculateQuote({ pricePerDay, days, discountPercent = 0, discountAmount = 0, discountType = 'percent' }) {
  ...
  if (discountType === 'npr_amount' && discountAmount > 0) {
    finalDiscountAmount = Math.min(discountAmount, subtotal); // Can't exceed subtotal
...
export async function createBooking(payload) {
  ...
  // For guest bookings, create a user profile automatically
  // PHONE NUMBER is the primary unique identifier for guest customers
  if (!userId && (payload.customer_phone || payload.customer_email)) {
    try {
      ...
      // First: Try to find by PHONE NUMBER (primary identifier)
      if (phoneKey) {
      ...
      // Second: Try to find by EMAIL if no phone match
      ...
      if (existingProfile) {
        // Found match - use existing profile
        userId = existingProfile.id;
        // Update phone/email if they're different/new
        const updates = {};
        ...
        // Create new user profile for guest
      ...
      // Continue without user_id if profile creation fails
    }
  }
  ...
  // Save to localStorage for guest users (so they can view their bookings)
  if (!userId && data) {
    try {
  ...
  // Create a notification for logged in user (if available)
  if (userId) {
    try {
