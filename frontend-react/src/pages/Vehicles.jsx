...
function formatNpr(n) { return `NPR ${Number(n || 0).toLocaleString()}`; }
...
export default function Vehicles() {
  ...
  const [availabilityMap, setAvailabilityMap] = useState({}); // Map of vehicleId -> boolean
  ...
  const filtered = useMemo(() => {
    let result = vehicles.filter(v => {
      // If dates are selected, use availability map; otherwise use status
      if (pickupDateTime && dropoffDateTime) {
        // If availability has been checked, use it; otherwise include all
        if (Object.keys(availabilityMap).length > 0 && availableOnly && !availabilityMap[v.id]) return false;
      } else {
        // No dates selected, fall back to status check
  ...
  return (
    <main className="vrs-theme-scope mx-auto w-[95%] max-w-[1460px] pb-14 pt-2 lg:pt-4">
      {/* Hero banner */}
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              {filtered.map((v, idx) => {
                ...
                return (
                  <AnimatedSection key={v.id} animation="fadeUp" delay={idx * 80}>
                  <div
                    onClick={() => navigate(`/vehicles/${toSlug(v.brand, v.name)}`)}
                    ...
                    <div className="relative h-52 overflow-hidden bg-gradient-to-br from-[#2a5c5f] to-[#1f5659]">
                      <img src={v.imageUrl || '/assets/images/car-transparent.png'} alt={v.name} loading="lazy"
                        ...
                        onError={e => { e.target.src = '/assets/images/car-transparent.png'; }} />
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex gap-2">
                        <button onClick={(e) => { e.stopPropagation(); navigate(`/vehicles/${toSlug(v.brand, v.name)}`); }}
