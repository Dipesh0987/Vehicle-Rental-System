    ...
    getVehicleBySlug(slug)
      .then(async (v) => {
        ...
        if (v?.id) {
          ...
          const allImages = imgs.length > 0 ? imgs.map((img) => img.image_url || img.url) : (v?.images?.length ? v.images : [v?.imageUrl]);
    ...
    try {
      ...
      if (result?.valid) { 
        ...
        const discountLabel = result.discount_type === 'npr_amount' 
          ? `NPR ${result.discount_amount?.toLocaleString()} discount applied!`
    ...
    return (
      <div className="min-h-screen flex items-center justify-center text-center">
        <div>
          ...
          <Link to="/vehicles" className="text-[13px] font-semibold text-accent hover:underline">Browse all vehicles</Link>
        </div>
      </div>
    );
  }
    ...
    <section className="detail-shell detail-stage relative z-10 mx-auto w-[95%] max-w-[1460px] space-y-6 rounded-[34px] p-5 sm:p-7 lg:p-9">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 text-[12px] font-semibold text-[#375053]">
        <div className="flex flex-wrap items-center gap-2">
          <Link to="/vehicles" className="rounded-full border border-[#d0dbd6] bg-white px-4 py-2 transition hover:-translate-y-[1px]">Back to Fleet Catalog</Link>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <article className="detail-card rounded-3xl border border-[#d5e0db] bg-white/90 p-5">
              ...
              <div className="mt-3 grid gap-2">
                {(vehicle.what_is_included || ['Insurance Coverage', 'Roadside Assistance', 'GPS Navigation', 'Free Cancellation (24h)']).map((item, idx) => (
            ...
            <article className="detail-card rounded-3xl border border-[#d5e0db] bg-white/90 p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {(vehicle.features || ['AC', 'Bluetooth', 'USB Charging', 'Backup Camera']).map((f, i) => (
          ...
          <aside className="detail-card rounded-3xl border border-[#d5e0db] bg-white/95 p-5 shadow-[0_16px_30px_rgba(10,31,34,0.08)] xl:sticky xl:top-5">
            ...
            <article className="mt-5 rounded-2xl border border-[#d6e2dd] bg-[#f8fcfa] p-4">
              ...
              <div className="space-y-2 text-[13px]">
                ...
                <div className="flex items-center justify-between"><span className="text-[#4a6569]">Tax</span><span className="font-semibold text-[#214346]">NPR {(quote?.tax || 0).toLocaleString()}</span></div>
