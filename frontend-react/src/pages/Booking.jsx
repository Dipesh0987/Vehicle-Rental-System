...
export default function Booking() {
    ...
    try {
      ...
      if (result?.valid) { 
        ...
        const discountLabel = result.discount_type === 'npr_amount' 
          ? `Coupon applied: NPR ${result.discount_amount?.toLocaleString()} off!`
          : `Coupon applied: ${result.discount_percent}% off!`;
  ...
  const handleReview = () => {
    ...
    if (!customerName || !customerEmail || !customerPhone) { setError('Please fill in all required fields (Name, Email, Phone).'); return; }
    if (available === false) { setError('Vehicle is not available for selected dates.'); return; }
    setReviewOpen(true);
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      const result = await createBooking({
        ...
        notes, coupon_code: couponCode || null, discount_percent: discountPercent, discount_amount: quote?.discountAmount || 0,
        base_amount: quote?.base || 0, service_fee: quote?.serviceFee || 0, total_amount: quote?.total || 0,
      });
      // Redirect directly to payment page
      navigate(`/payment?booking=${result.id}`);
    } catch (err) { setError(err.message); setReviewOpen(false); } finally { setSubmitting(false); }
  };
      ...
      <section className="booking-form-shell rounded-[34px] border border-[rgba(23,57,60,0.14)] bg-[linear-gradient(150deg,rgba(255,255,255,0.93),rgba(246,239,229,0.84))] p-5 shadow-[0_24px_52px_rgba(10,31,34,0.1)] backdrop-blur-sm sm:p-7">
        ...
        <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link to="/vehicles" className="rounded-full border border-[#d0dbd6] bg-white px-4 py-2 text-[13px] font-semibold text-[#264447] transition duration-200 hover:-translate-y-[1px] hover:bg-[#f3f8f6] hover:text-[#183b3f]">
        ...
        <div className="grid gap-5 lg:grid-cols-[1.35fr,0.95fr] lg:items-start">
          ...
          <section className="booking-form-card space-y-4 rounded-3xl border border-[#d5e0db] bg-[linear-gradient(165deg,#ffffff,#f9f4eb)] p-4 shadow-[0_14px_30px_rgba(10,31,34,0.08)] sm:p-5">
            ...
            {/* Row 1: Vehicle / Pickup Time / Driver */}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="grid gap-1">
                <span className="text-[12px] font-semibold text-[#315154]">Start Date</span>
                <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} min={today} required className={inputCls} />
              </label>
              <label className="grid gap-1">
                <span className="text-[12px] font-semibold text-[#315154]">End Date</span>
                <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} min={startDate || today} required className={inputCls} />
              </label>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="grid gap-1">
                ...
                <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} required placeholder="Enter your full name" className={inputCls} />
              </label>
              <label className="grid gap-1">
                <span className="text-[12px] font-semibold text-[#315154]">Email (Optional)</span>
                <input type="email" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} placeholder="you@example.com" className={inputCls} />
              </label>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 sm:items-start">
              <label className="grid gap-1 content-start">
                ...
                <input type="tel" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} required placeholder="+977 555 000 000" className={inputCls} />
              </label>
              <label className="grid gap-1 content-start">
            ...
            {/* Notes */}
            <label className="grid gap-1">
              <span className="text-[12px] font-semibold text-[#315154]">Pick Up Location (Optional)</span>
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} placeholder="Enter pick-up location"
      ...
      <div className={`fixed inset-0 z-[260] flex items-center justify-center bg-[rgba(8,22,24,0.62)] px-4 transition duration-200 ${reviewOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="booking-review-card w-full max-w-[560px] rounded-[26px] border border-[rgba(23,57,60,0.18)] bg-[linear-gradient(155deg,#ffffff,#f6efe4)] p-5 shadow-[0_30px_80px_rgba(2,14,16,0.46)] sm:p-6">
          ...
          <div className="booking-review-summary mt-3 rounded-2xl border border-[#d8e3de] bg-white p-4 text-[13px] text-[#375c5f] shadow-[0_10px_20px_rgba(10,31,34,0.08)] space-y-2">
            ...
            <div className="flex justify-between rounded-xl border border-[#e4ece8] px-3 py-2"><span className="font-semibold">Pick Up Location</span><span>{notes || '-'}</span></div>
            <div className="grid gap-2 sm:grid-cols-2">
