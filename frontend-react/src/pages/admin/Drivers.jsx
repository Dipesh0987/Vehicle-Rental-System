...
const availColor = (s) => {
  const c = { 'Available': 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300', 'On Trip': 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300', 'Off Shift': 'bg-slate-100 text-slate-600 dark:bg-slate-500/20 dark:text-slate-300', 'On Leave': 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300' };
  return c[s] || c['Available'];
};
const licenceColor = (s) => {
  const c = { 'Valid': 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300', 'Expired': 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-300', 'Suspended': 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-300', 'Pending Verification': 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300' };
...
export default function AdminDrivers() {
  ...
  const openDetail = (d) => { setDetail(d); setMode('detail'); };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (mode === 'edit' && detail) {
        await supabase.from('drivers').update(payload).eq('id', detail.id);
      } else {
