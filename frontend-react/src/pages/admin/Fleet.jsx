...
export default function AdminFleet() {
  ...
  const [activeTab, setActiveTab] = useState('all'); // 'all', 'available', 'ontrip'
  const [filterDate, setFilterDate] = useState(new Date().toISOString().split('T')[0]);
  const [availableVehicles, setAvailableVehicles] = useState([]);
  const [onTripVehicles, setOnTripVehicles] = useState([]);
  ...
  useEffect(() => {
    processVehicles();
  }, [vehicles, bookings, filterDate]);

  const fetchData = async () => {
    setLoading(true);
    
    // Fetch vehicles and bookings separately to avoid complex join issues
    const { data: vehData } = await supabase.from('vehicles').select('*, vehicle_images(*)').order('name');
    const { data: bookData, error: bookError } = await supabase.from('vehicle_bookings').select('*').order('start_date');
    ...
    console.log('All bookings fetched:', bookData?.length || 0);
    console.log('Booking statuses:', bookData?.map(b => ({ id: b.id, status: b.status, start: b.start_date, end: b.end_date })));
    
    // Filter bookings in code to avoid OR query issues
    const activeBookings = (bookData || []).filter(b => 
      b.status === 'confirmed' || b.status === 'active'
    );
    
    console.log('Active bookings (confirmed + active):', activeBookings.length);
  ...
  const processVehicles = () => {
    // Ensure selectedDate is in YYYY-MM-DD format for comparison
    const selectedDateStr = filterDate;
    
    // Find vehicles currently on trip (booked for the selected date)
    ...
    bookings.forEach(b => {
      ...
      // For 'active' status: always show in on-trip (regardless of date)
      // For 'confirmed' status: show if date overlaps with selected date
      const isActiveStatus = b.status === 'active';
      const isConfirmedStatus = b.status === 'confirmed';
      const dateOverlaps = startDateStr <= selectedDateStr && endDateStr >= selectedDateStr;
      
      // A booking is "on trip" if:
      // 1. Status is 'active' (currently on trip), OR
      // 2. Status is 'confirmed' AND dates overlap with selected date
      const isOnTrip = isActiveStatus || (isConfirmedStatus && dateOverlaps);
      
      // Debug logging for first few bookings
      if (bookings.indexOf(b) < 5) {
        console.log(`Booking ${b.id}: start=${startDateStr}, end=${endDateStr}, selected=${selectedDateStr}, status=${b.status}, isOnTrip=${isOnTrip}`);
      }
      
      if (isOnTrip) {
        bookedVehicleIds.add(b.vehicle_id);
        // Enrich booking with vehicle info from vehicles array
        const vehicle = vehicles.find(v => v.id === b.vehicle_id);
        currentTrips.push({ ...b, vehicles: vehicle || null });
      }
    });
    
    console.log('On-trip vehicles found:', currentTrips.length);
    console.log('Trips with vehicles:', currentTrips.map(t => ({ id: t.id, vehicle: t.vehicles?.name })));
    
    // Available vehicles (not in bookedVehicleIds and status is available)
  ...
  const statusDot = (s) => {
    const c = { available: 'bg-emerald-500', maintenance: 'bg-amber-500', unavailable: 'bg-slate-400', inactive: 'bg-rose-500', booked: 'bg-blue-500' };
    return c[s] || 'bg-slate-400';
  };

  const fmtDate = (d) => d ? new Date(d).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '-';

  const groups = vehicles.reduce((acc, v) => {
    ...
    <div className="space-y-4">
      ...
      <section className={`${panel} p-4`}>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          <div>
            <label className="text-xs font-semibold">Filter Date</label>
            <input 
              type="date" 
              value={filterDate}
          ...
          <div className="flex items-end gap-2">
            <button 
              ...
              className={`flex-1 rounded-xl px-3 py-2 text-sm font-semibold transition ${activeTab === 'all' ? 'bg-[#1f7668] text-white' : 'border border-slate-200 dark:border-white/10'}`}
            >
          ...
          <div className="flex items-end gap-2">
            <button 
              ...
              className={`flex-1 rounded-xl px-3 py-2 text-sm font-semibold transition ${activeTab === 'available' ? 'bg-emerald-600 text-white' : 'border border-slate-200 dark:border-white/10'}`}
            >
          ...
          <div className="flex items-end gap-2">
            <button 
              ...
              className={`flex-1 rounded-xl px-3 py-2 text-sm font-semibold transition ${activeTab === 'ontrip' ? 'bg-blue-600 text-white' : 'border border-slate-200 dark:border-white/10'}`}
            >
          ...
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Showing: All <span className="font-semibold text-blue-600">Active</span> bookings + <span className="font-semibold text-emerald-600">Confirmed</span> for {fmtDate(filterDate)}
              </p>
            </div>
                  ...
                  {onTripVehicles.map((trip) => (
                    ...
                    >
                      ...
                      <td className="py-3 pr-3 text-xs font-semibold text-amber-600 dark:text-amber-400">
                        {trip.end_date === filterDate ? 'Returning Today' : fmtDate(trip.end_date)}
                      </td>
                      <td className="py-3 pr-3">
        ...
        <section className={`${panel} p-4 sm:p-5`}>
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-base font-extrabold">
              ...
              Available Vehicles for {fmtDate(filterDate)}
            </h3>
