...
// Roles must match database check constraint user_profiles_role_check
...
export default function AdminRoles() {
  ...
  const existingSuperAdmin = admins.find(a => a.role === 'super_admin' && a.id !== currentUser?.id);

  const updateRole = async (id, newRole) => {
    // Prevent creating multiple super admins via role change
  ...
  const handleCreateAccount = async (e) => {
    ...
    if (createForm.role === 'super_admin' && admins.some(a => a.role === 'super_admin')) {
    ...
    try {
      ...
      if (authError) {
        ...
        if (authError.message?.includes('already registered')) {
          // Query user_profiles to find existing user by email
          ...
          if (existingProfiles && existingProfiles.length > 0) {
            ...
            // Update the profile directly without signing in
        ...
        // User created successfully (even if authData.user is null due to email confirmation)
        ...
        if (existingProfile && existingProfile.length > 0) {
          ...
          profileError = error;
          console.log('Updated existing profile for:', email);
        } else {
  ...
  const handleChangePassword = async (e) => {
    ...
    setSaving(true);
    try {
      // We need to use admin API or custom function to change another user's password
      // For now, we'll use a workaround by creating a stored procedure
      ...
      if (error) {
        // Fallback: If RPC doesn't exist, show instructions
